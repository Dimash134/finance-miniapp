...
# ---------- СВОД (главная: три блока) ----------
@app.route('/svod')
def svod():
    try:
        ws = spreadsheet.worksheet("Свод")
        p1 = ws.get("A2:B5")    # блок 1
        p2 = ws.get("D2:E7")    # блок 2
        p3 = ws.get("G2:H12")   # блок 3
        return jsonify({"p1": p1, "p2": p2, "p3": p3})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# ---------- СВОД: кнопка (B6) + детализация трёх блоков ----------
@app.route('/svod-details')
def svod_details():
    try:
        ws = spreadsheet.worksheet("Свод")
        b6 = ws.acell("B6").value
        part1 = ws.get("A18:B22")
        part2 = ws.get("A32:B36")
        part3 = ws.get("A46:B50")
        return jsonify({"b6": b6, "part1": part1, "part2": part2, "part3": part3})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def breakdown():
    """
    Параметры:
      branch: Private/Highschool/Academy
      scope:  'текущий' | 'месяц' | 'дата'
      page, limit, search
    Источники:
      Листы: "Расшифровка ДДС сегодня" / "Расшифровка ДДС на месяц" / "Расшифровка ДДС на дату"
      Колонки:
        B — сумма, F — статья, D — контрагент, E — назначение
        строки 3..1000
    """
    try:
        branch = request.args.get("branch", "Private")
        ...
        key = DDS_SOURCES.get(branch, DDS_SOURCES["Private"])["key"]
        ...
        ws = book.worksheet(scope_map.get(scope, scope_map["текущий"]))
        # считываем диапазоны столбцов разом
        colB = ws.get("B3:B1000")
        colF = ws.get("F3:F1000")
        colD = ws.get("D3:D1000")
        colE = ws.get("E3:E1000")

        items = []
        n = max(len(colB), len(colF), len(colD), len(colE))
        for i in range(n):
            amount = (colB[i][0] if i < len(colB) and colB[i] else "").strip()
            article = (colF[i][0] if i < len(colF) and colF[i] else "").strip()
            counter = (colD[i][0] if i < len(colD) and colD[i] else "").strip()
            purpose = (colE[i][0] if i < len(colE) and colE[i] else "").strip()
            if not (amount or article or counter or purpose):
                continue
            row = {"amount": amount, "article": article, "counterparty": counter, "purpose": purpose}
            if search:
                hay = " ".join([amount, article, counter, purpose]).lower()
